...
# --- 2. XỬ LÝ CREDENTIALS (NGOÀI CACHE) ---
if "gcp_service_account" in st.secrets:
    ...
    try:
        ...
        st.error("Thiếu file credentials.json hoặc Secrets chưa được cấu hình!")
        st.stop()


# --- 3. QUẢN LÝ KẾT NỐI (SỬA LỖI CACHEDWIDGETWARNING) ---
...
# --- 4. QUẢN LÝ ĐĂNG NHẬP (KHÔNG DÙNG CACHE CHO COOKIE) ---

# XÓA @st.cache_resource Ở ĐÂY VÌ COOKIEMANAGER LÀ MỘT WIDGET
...
def check_login():
    if 'user_info' in st.session_state:
        return True
    # Lấy cookie (hàm này sẽ chạy mỗi lần rerun để kiểm tra)
...
if not check_login():
    ...
    with st.container(border=True):
        ...
        u_name = st.text_input("Họ và tên", key="login_name")
...
def render_form(type_f="XUẤT"):
    ...
    if search_term:
        ...
        if not f.empty:
            for idx, row in f.head(3).iterrows():
                if st.button(f"📍 {row['MaQua']} - {row['TenQua']}", key=f"btn_{type_f}_{idx}",
    ...
    if curr_ma:
        is_new = curr_ma not in df_g['MaQua'].tolist() if not df_g.empty else True
        ton = get_current_stock(curr_ma) if not is_new else 0
        st.success(f"Đang chọn: **{curr_ten}** | Tồn: **{ton}**")

        with st.form(f"form_submit_{type_f}", clear_on_submit=True):
            ...
            if st.form_submit_button(f"XÁC NHẬN {type_f}", use_container_width=True):
                if so_ct:
                    ...
                    new_record = {
                        ...
                        "MaQua": curr_ma, "TenQua": curr_ten,
                        "SoLuong": sl if type_f == "NHẬP" else -sl,
                        "SoChungTu": so_ct, "NguoiThucHien": st.session_state['user_info']['name'],
                        "GhiChu": note
                    }
                    save_data_to_gsheet(pd.concat([df_t, pd.DataFrame([new_record])], ignore_index=True),
...
with tabs[3]:
    ...
    df_history = load_data_from_gsheet("nhatky_xuatnhap", CREDS_DATA)
